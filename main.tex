...
\begin{explainframe}{Fully Optimized Keyword Arguments}
\begin{itemize}
    \item Example shows simplified case without any regular/optional arguments.
    \item Layout of child nodes: regular argument nodes (no optional args), keyword arguments (including optional args as \lstinline{MissingKeywordArgumentNode} if not present), marker node (indicating optimized call).
\end{itemize}
\end{explainframe}
...
\begin{table}
\centering
Keyword arguments are as fast as regular arguments.
\end{table}
...
\begin{itemize}
 ...
 \item Large AST interpreters can still get unclear/distracting (it is Java $\rightarrow$ some code needed to get things done)
